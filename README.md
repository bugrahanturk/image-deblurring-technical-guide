...
2.  **Non-Uniform Motion Blur**: Arises when different objects or parts of the scene move with varying velocities and/or directions. The PSF is spatially varying (k<sub>x,y</sub>).
...
| :--- | :--- | :--- |
| **Uniform Motion Blur** | PSF is a line segment of length L and orientation θ. | Camera (or entire scene) moves at constant velocity; same PSF for all pixels. |
| **Non-Uniform Motion Blur** | B(x,y) = Σ<sub>u,v</sub> I(u,v) k<sub>x,y</sub>(x-u, y-v) + n(x,y) | Objects move in different directions or speeds; each pixel/region has its own PSF k<sub>x,y</sub>. |
| **Defocus Blur** | k(x, y) = 1/(πR²) if x² + y² ≤ R², 0 otherwise | Lens focusing error results in out-of-focus regions; PSF is a circular disk of radius R. |
| **Atmospheric/Turbulence** | Typically Gaussian: k(x,y) ∝ exp(-(x²+y²)/(2σ²)) plus speckle noise. | Optical distortions due to heat or air currents; common in long-range or astronomical imaging. |

*   **Blind Deblurring**: Kernel `k` is unknown; the model must estimate both k̂ and Î.
...
*   **Input:** 224 × 224 blurred image (RGB).
*   **Layers:**
    1.  `Conv1`: 64 filters, 9 × 9 kernel, ReLU.
    2.  `Conv2`: 32 filters, 1 × 1 kernel, ReLU.
    3.  `Conv3`: 3 filters, 5 × 5 kernel, Linear output.
*   **Loss Function:** Mean Squared Error (MSE).
*   **Optimizer:** Adam (lr = 10⁻³).

#### Training Setup
*   **Dataset:** Kaggle Blur Dataset (1,050 images).
*   **Data Augmentation:** Flips, crops to 224 × 224.
*   **Batch Size:** 16
*   **Epochs:** 40
...
*   **Loss Functions:**
    1.  **Perceptual Loss (L<sub>perc</sub>):** MSE on VGG-19 feature maps.
        
        L<sub>perc</sub> = Σ<sub>i</sub> ||φ<sub>i</sub>(G(B)) - φ<sub>i</sub>(I)||²
        
    2.  **Adversarial Loss (L<sub>adv</sub>):** Wasserstein GAN with Gradient Penalty (WGAN-GP).
*   **Total Generator Loss:**
    
    L<sub>G</sub> = λ<sub>perc</sub> L<sub>perc</sub> + λ<sub>adv</sub> L<sub>adv</sub>
...
*   **Loss Function:** The sum of MSE losses calculated at each scale.
    ...
    L = Σ<sub>s=1</sub><sup>S</sup> ||I<sub>s</sub> - Î<sub>s</sub>||²
...
#### Architecture Details
1.  **BGAN (Blur-GAN):** A CycleGAN-like network trained on unpaired sharp and real blurry images to learn a realistic blur synthesis function G<sub>B</sub>: I → B<sub>syn</sub>.
2.  **DBGAN (Deblur-GAN):** A standard deblurring GAN (similar to DeblurGAN) trained on paired data (B<sub>syn</sub>, I) generated by BGAN.
